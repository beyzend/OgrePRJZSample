...
CSMGpuConstants::CSMGpuConstants(size_t cascadeCount)
{
	...
	mParamsShadowMatrix = GpuProgramManager::getSingletonPtr()->getSharedParameters("params_shadowMatrix");
...
void CSMGpuConstants::updateCascade(const Ogre::Camera &texCam, size_t index)
{
	if (index == 0)
	{
		...
		Matrix4 texMatrix0 = PROJECTIONCLIPSPACE2DTOIMAGESPACE_PERSPECTIVE * texCam.getProjectionMatrix() * mFirstCascadeViewMatrix;
