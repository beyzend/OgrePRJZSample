...
void HDRCompositor::notifyMaterialRender(Ogre::uint32 pass_id, Ogre::MaterialPtr &mat)
{
    ...
    {
    case MID_KEY:
        def = mat->getTechnique(0)->getPass(0)->getFragmentProgramParameters()->_findNamedConstantDefinition("Key");
        if(def)
            mat->getTechnique(0)->getPass(0)->getFragmentProgramParameters()->_writeRawConstant(def->physicalIndex, m_Key);
        break;

    case MID_BRIGHT:
        def = mat->getTechnique(0)->getPass(0)->getFragmentProgramParameters()->_findNamedConstantDefinition("BRIGHT_PASS_THRESHOLD");
        if(def)
    ...
    case MID_ADAPT:
        def = mat->getTechnique(0)->getPass(0)->getFragmentProgramParameters()->_findNamedConstantDefinition("dTime");
        if(def)
    ...
    case MID_FINAL:
        if(m_ToneMapper != TM_NONE && mat->getTechnique(0)->getPass(0)->hasFragmentProgram())
        {
...
String HDRCompositor::ToneMapperToString(const TONEMAPPER ToneMapper)
{
    ...
    case TM_REINHARDS:
        return "Reinhard's";
    case TM_REINHARDSMOD:
        return "Modified Reinhard's";
    case TM_LOG:
        return "Logarithmic";
    case TM_ADAPTLOG:
        return "AdaptiveLog";
    case TM_REINHARDLOCAL:
        return "Reinhard(Local)";
    default:
        return "None";
    }
}
